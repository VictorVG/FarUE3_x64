--[[
  Mantis#0002675: Symlink создается только с абсолютным путем. Нет возможности задать относительный путь
  скрипт SVN r11865, работает только при 2-х открытых файловых панелях с текущим файлом/каталогом.

  Внимание! Данная версия скрипта предназначена для работы совместно с Far 3.0 b3924 r11865 и новее!

  Для управления типом создаваемого линка используются чекбоксы:

  [x]Symlink : [x] создаются симлинки на каталог/файл, [ ]Symlink - создаются junction/hardlink
  [ ]Absolute Path : по умолчанию сброшен, если [x]Symlink установлен и линк находится в пределах тома,
     то при [ ]Absolute Path создаются относительные симлинки, если [x]Absolute Path то создаются
     абсолютные симлинки как при вызове AltF6. На другой том всегда cоздаются абсолютные симлинки.
