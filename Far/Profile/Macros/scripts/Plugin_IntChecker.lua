...
-- v1.8.1 - возвращаемый хэш всегда строка, все сообщения на английском, при ошибочном Target
-- выводим хэш на экран, рефакторинг;
-- 13.10.2019 16:32:37 +0300
...
-- Логика анализатора:
--
-- если задан каталог, то считаем от него и dn == полный путь к нему, fn == его имя,
-- если задан файл, то dn == путь к его родительскому каталогу,
...
local ICId,Quiet="E186306E-3B0D-48C1-9668-ED7CF64C0E65",true;
   return mf.string(Plugin.SyncCall(ICId,"gethash",""..hn.."",""..pth.."",1));
end
 if md == 4 then
...
Macro{
 ...
 if mf.msgbox("Question to the user:",Msg,0x00020000) == 1
        then
          Keys("Enter");
        else
...
Macro{
 ...
 if mod == "u" then
   ...
   if err == 0 then
    if md == 3 then
      if tg == "f" then hsum=dn.."\\".."hashsum"..ext; end;
     elseif md == 2 then
     if tg == "f" then  hsum=dn.."\\"..mf.fsplit(fn,4)..ext; end;
    end;
   else
   ...
   if mod == "d" then
     md=4;
     if tg == "f" then dn=APanel.Path0;
     fn=APanel.Path0..hashsum..ext; end;
     er=0;
    elseif (mod == "a" or mod == "s") then
     md=1;
     if tg == "f" then
        dn=APanel.Path0;
        hsum=APanel.Path0.."hashsum"..ext;
     end;
     er=0;
    else
     md=0;
     if tg == "f" then
        dn=APanel.Path;
        hsum=APanel.Path0..mf.fsplit(fn,4)..ext;
     end;
     er=0;
   end;
 end;
